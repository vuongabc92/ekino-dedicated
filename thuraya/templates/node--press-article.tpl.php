...
$lang_article=@$node->field_article_language['und']['0']['value'];
if($lang_article=='ar'){$lang_class="content-ar" ; } else { $lang_class="content-en" ; }
?>
  ...
  <div class="colums artical">
	<div class="col1">
	  <div class="in-container">
		<?php 
			$noOfImages = count($node->field_article_image[$node->language]);
			if($noOfImages > 0):
			if($browDetails['version'] == "8.0" && $browDetails['name'] == "Internet Explorer"):
		?>
		<div><img src="<?php print file_create_url($node->field_article_image[$node->language][0]['uri']) ?>" alt="Press"/></div>
		<?php else: ?>
		<div class="circleBg">
		<div class="circle-img" ><img src="<?php print file_create_url($node->field_article_image[$node->language][0]['uri']); ?>" alt="Press"/></div>
		</div>
		<?php endif; ?>
		<?php endif; ?>
	...
	<div class="col2">
	  ...
	  <div class="in-container">
		<?php if ($title): ?>
			<h1 class="<?php print $lang_class; ?>"><?php print $title; ?></h1>
		<?php endif; ?>
		<?php if ($node->field_short_description): ?>
		<h2 class="<?php print $lang_class; ?>"><?php print $node->field_short_description[$node->language][0]['safe_value']; ?></h2>
		<?php endif; ?>
		<!--  Prese news -->
		
		<?php if($node->body): ?>
		<article class="<?php print $lang_class; ?>">
		...
		<div class="pd-footer">
		  <ul>
			<?php for($i=0; $i<$downloadFiles;$i++) {
				//$fileDesc = "Download Press Release ".($i+1);
				$fileDesc = "Download Press Release";
				?>
		...
		<div class="sn-links"><a href="javascript:history.go(-1);" class="anchor new-medium" ><span class="ui-icon prev"></span><span>Back</span></a>
		  <ul class="ul-sn">
			...
			<li><a href="http://www.facebook.com/sharer.php?u=<?php print curPageURL();?>" target="_blank" class="ui-icon facebook"></a>
